...
function assertVersions() {
  ...
  if (!nodeV.startsWith(EXPECTED.node)) {
    console.error(`ERROR: Node version mismatch. Expected prefix ${EXPECTED.node} but got ${nodeV}`);
    process.exit(1);
  }
  if (pnpmV !== EXPECTED.pnpm) {
    console.error(`ERROR: pnpm version mismatch. Expected ${EXPECTED.pnpm} but got ${pnpmV}`);
    process.exit(1);
  }
}
